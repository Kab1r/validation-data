...
class Parser:
    ...
    def __init__(self, file):
        ...
        self.__macho = {}
        self.__output: dict[str, Any] = {"name": "IMDAppleServices"}
    ...
    def identify_file(self):
        ...
        if magic in mdictionary.machos:
            ...
            raise ValueError("Provided file has unrecognized magic: {}".format(magic))
        ...
        while i < 28:
            if (0x1 & (flags >> i)) == 0x1:
                if 2**i in mdictionary.flags:
                    ...
                    self.add_abnormality('Unknown mach-o flag "{}".'.format(2**i))
        ...
        for _ in range(nsects):
            if self.__file.tell() + sect_size > m_offset + m_size:
                self.add_abnormality(
                    'Section at offset "{}" with size "{}" '
                    "greater than mach-o size.".format(self.__file.tell(), sect_size)
                )
    ...
    def parse_symseg(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "offset": offset, "size": size}
    ...
    def parse_thread(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "state": state, "count": count}
    ...
    def parse_ident(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "strings": []}
    ...
    def parse_prepage(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size}
    ...
    def parse_load_dylinker(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "name": self.get_string()}
    ...
    def parse_twolevel_hints(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "offset": offset, "nhints": nhints}
    ...
    def parse_prebind_cksum(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "cksum": cksum}
    ...
    def parse_uuid(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "uuid": UUID(bytes=uuid).hex}
    ...
    def parse_rpath(self, cmd, cmd_size):
        ...
        output = {"cmd": cmd, "cmd_size": cmd_size, "path": path}
    ...
    def parse_lcs(self, offset, size, nlcs, slcs):
        ...
        for _ in range(nlcs):
            ...
            if self.__is_64_bit and cmd_size % 8 != 0:
                raise ValueError(
                    'Load command size "{}" for 64-bit mach-o at '
                ...
                raise ValueError(
                    'Load command size "{}" for 32-bit mach-o at '
            ...
            if cmd == "SEGMENT" or cmd == "SEGMENT_64":
                ...
                self.__macho["lcs"].append(self.parse_load_dylib(cmd, cmd_size))
            elif cmd in ("LOAD_DYLINKER", "ID_DYLINKER", "DYLD_ENVIRONMENT"):
                self.__macho["lcs"].append(self.parse_load_dylinker(cmd, cmd_size))
            elif cmd == "PREBOUND_DYLIB":
                self.__macho["lcs"].append(self.parse_prebound_dylib(cmd, cmd_size))
            elif cmd in ("ROUTINES", "ROUTINES_64"):
                self.__macho["lcs"].append(self.parse_routines(cmd, cmd_size))
            elif cmd in ("SUB_FRAMEWORK", "SUB_UMBRELLA", "SUB_CLIENT", "SUB_LIBRARY"):
    ...
    def parse_syms(self, offset, size, lc_symtab):
        ...
        if lc_symtab["symoff"] > size:
            self.add_abnormality(
                'Symbol table at offset "{}" out of '
                "bounds.".format(offset + lc_symtab["symoff"])
            )
        ...
        entropy = self.calc_entropy(self.__file.read(lc_symtab["nsyms"] * symbol_size))

        if entropy >= 0.8:
            self.add_abnormality(
                'Symbol table with entropy of "{}" is '
        ...
        if lc_symtab["symoff"] + lc_symtab["nsyms"] * symbol_size > size:
            self.add_abnormality(
                'Symbol table at offset "{}" partially out '
    ...
    def parse_strings(self, offset, size, lc_symtab):
        ...
        if entropy >= 0.8:
            self.add_abnormality(
                'String table with entropy of "{}" is '
                "probably packed. Not attempting to "
                "parse.".format(entropy)
            )
        ...
        if true_offset + lc_symtab["strsize"] > offset + size:
            self.add_abnormality(
                'String Table at offset "{}" partially out '
                "of bounds. Attempting to parse as many "
    ...
    def parse_imports(self, offset, size, lc_symtab, lc_dysymtab=None, lc_dylibs=None):
        ...
        if lc_dysymtab is not None:  # Use symtab layout info from DYSYMTAB
            ...
            undef_syms = filter(
                lambda sym: sym["n_type"] in ("UNDF", "PBUD"), self.__macho["symtab"]
            )
    ...
    def parse_certs(self, sig_offset, index_offset):
        ...
        if magic != mdictionary.signatures["BLOBWRAPPER"]:
            self.add_abnormality(
                'Bad magic "{}" for certificate blob wrapper '
    ...
    def parse_codedirectory(self, sig_offset, index_offset):
        ...
        page_size = int(round(exp(int(self.__file.read(1).hex(), 16) * log(2))))
        ...
        self.__file.seek(true_offset + hash_offset - n_special_slots * hash_size)
    ...
    def to_oid(self, length):
        ...
        data_bytes = [int(self.__file.read(1).hex(), 16) for i in range(length)]
        ...
        if operator == "False":
            ...
            if in_or:
                expression += "({} or {})".format(
                    self.parse_expression(in_or=True), self.parse_expression(in_or=True)
                )
            else:
                expression += "{} or {}".format(
                    self.parse_expression(in_or=True), self.parse_expression(in_or=True)
    ...
    def parse_sig(self, offset, size, lc_codesig):
        ...
        if lc_codesig["dataoff"] + lc_codesig["datasize"] > size:
            self.add_abnormality(
                'CODE_SIGNATURE at offset "{}" with size '
                '"{}" greater than mach-o size.'.format(
                    offset + lc_codesig["dataoff"], lc_codesig["datasize"]
                )
            )
        ...
        if magic != mdictionary.signatures["EMBEDDED_SIGNATURE"]:
            self.add_abnormality(
                'Bad magic "{}" for embedded signature at '
        ...
        for _ in range(count):
            ...
            if index_type in mdictionary.indeces:
                ...
                self.add_abnormality(
                    'Unknown code signature index type "{}" '
                    'at offset "{}".'.format(index_type, self.__file.tell() - 4)
                )
    ...
    def parse_macho(self, offset, size):
        ...
        if cputype in mdictionary.cputypes:
            if subtype in mdictionary.cputypes[cputype]:
                ...
                self.add_abnormality(
                    'Unknown SUBTYPE "{}" for CPUTYPE "{}" '
                    'at offset "{}".'.format(subtype, cputype, offset + 8)
                )
        ...
        if "symtab" in self.__macho and "strtab" in self.__macho:
            ...
            if "TWOLEVEL" in self.__macho["flags"]:
                lc_dylibs = list(
                    filter(lambda x: x["cmd"].endswith("DYLIB"), self.__macho["lcs"])
                )

            self.parse_imports(
                offset, size, lc_symtab, lc_dysymtab=lc_dysymtab, lc_dylibs=lc_dylibs
            )
    ...
    def parse_universal(self):
        ...
        self.__output["universal"] = {"machos": []}
...
class mdictionary:
    cert_slots = {-1: "root", 0: "leaf"}

    hashes = {0: "No Hash", 1: "SHA-1", 2: "SHA-256"}

    segment_flags = {1: "HIGHVM", 2: "FVMLIB", 4: "NORELOC", 8: "PROTECTED_VERSION_1"}

    n_types = {0x0: "UNDF", 0x2: "ABS", 0xE: "SECT", 0xC: "PBUD", 0xA: "INDR"}
    ...
    cputypes = {
        -1: {-2: "ANY", -1: "MULTIPLE", 0: "LITTLE_ENDIAN", 1: "BIG_ENDIAN"},
        1: {
            -2: "VAX",
            -1: "MULTIPLE",
            0: "VAX_ALL",
        ...
        10: {-2: "MC98000", -1: "MULTIPLE", 0: "MC98000_ALL", 1: "MC98601"},
        11: {-2: "HPPA", -1: "MULITPLE", 0: "HPPA_ALL or HPPA_7100", 1: "HPPA_7100LC"},
        12: {
            -2: "ARM",
            -1: "MULTIPLE",
            0: "ARM_ALL",
        ...
        15: {-2: "I860 (big-endian)", -1: "MULTIPLE", 0: "I860_ALL", 1: "I860_860"},
        18: {
            -2: "POWERPC",
            -1: "MULTIPLE",
            0: "POWERPC_ALL",
